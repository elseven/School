...
/**
 * Tests encoding and decoding operations for the ReliableTransport message
 * class
 * 
 * @author drdan
 * 
...
public class ReliableTransportMessageTest extends TestCase {
	...
	public void testPaddedInt() {
		...
		assertArrayEquals("encoding of 345", expected1,
				ReliableTransportMessage.leftPaddedInt(345, 5));
		char[] expected2 = new String("  -22").toCharArray();
		assertArrayEquals("encoding of -22", expected2,
				ReliableTransportMessage.leftPaddedInt(-22, 5));
		try {
			char[] bogus = ReliableTransportMessage.leftPaddedInt(345, 2);
	...
	public void testConstructor() throws UnknownHostException {
		...
		ReliableTransportMessage msg = new ReliableTransportMessage(srcIP,
				destIP, 2200, 2015, ReliableTransportMessage.DATA, 1, "Hi mom!");
		...
		assertEquals("payload", "Hi mom!", msg.getPayload().trim());
	}
	...
	public void testIPencoding() throws UnknownHostException {
		InetAddress srcIP = InetAddress.getByName("172.17.152.122");
		InetAddress destIP = InetAddress.getByName("172.17.152.124");
		assertEquals("src ip encoding", " 172.17.152.122", new String(
				ReliableTransportMessage.leftPaddedIP(srcIP, 15)));
		assertEquals("dest ip encoding", " 172.17.152.124", new String(
				ReliableTransportMessage.leftPaddedIP(destIP, 15)));
	}
	...
	public void testEncoding() throws UnknownHostException {
		...
		assertEquals("Header length", ReliableTransportMessage.HEADER_LEN,
				expectedHeader.length());
		assertEquals("padding length", 23, padding.length());
		InetAddress srcIP = InetAddress.getByName("172.17.152.122");
		InetAddress destIP = InetAddress.getByName("172.17.152.17");
		ReliableTransportMessage msg = new ReliableTransportMessage(srcIP,
				destIP, 53200, 2016, ReliableTransportMessage.DATA, 1, payload);
		assertEquals("source IP", "/172.17.152.122", msg.getSourceIP()
				.toString());
		assertEquals("dest IP", "/172.17.152.17", msg.getDestIP().toString());
		msg.encode();
		String expectedEncoding = expectedHeader + payload + padding
				+ checksumStr;
		assertEquals("Expected Encoding length", 78, expectedEncoding.length());
		assertEquals("Checksum input length", 73,
				ReliableTransportMessage.HEADER_LEN
						+ ReliableTransportMessage.PAYLOAD_LEN);
		for (int i = 0; i < 73; i++)
			// check i-th term of checksum
			assertEquals("Checksum term " + i,
					(int) expectedEncoding.charAt(i), msg.getChecksumTerm(i));
	...
	public void testReconstituteMessage() {
		String encoding1 = " 172.17.152.12253200  172.17.152.17 2017D 1TCP, the Queen of Protocols!   4587";
		...
		assertEquals("encoding1 length", ReliableTransportMessage.BUFFER_LEN,
				encoding1.length());
		ReliableTransportMessage msg1 = ReliableTransportMessage
				.reconstitute(encodingBytes);
		assertNotNull("reconstituted message 1 is NULL", msg1);
		assertEquals("Source IP", "/172.17.152.122", msg1.getSourceIP()
				.toString());
		...
		assertEquals("Payload length", ReliableTransportMessage.PAYLOAD_LEN,
				msg1.getPayload().length());
		assertEquals("Payload", "TCP, the Queen of Protocols!  ",
				msg1.getPayload());
		...
		String encoding2 = " 172.17.152.12253200  172.17.152.17 2017D 1Drink Coca-cola!               4568";
		assertEquals("encoding2 length", ReliableTransportMessage.BUFFER_LEN,
				encoding2.length());
		...
		ReliableTransportMessage msg2 = ReliableTransportMessage
				.reconstitute(encodingBytes2);
